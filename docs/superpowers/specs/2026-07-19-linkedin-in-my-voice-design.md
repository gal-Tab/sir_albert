...
2. **Posts Gal admires** — Gal supplied 4 Hebrew posts (all by *others*) as the
   target aesthetic. We extract structure and texture only, never their words, and
   never treat any one author's tics as Gal's voice. Two registers emerged:
   - **Personal-builder** (the core): pain-or-spark hook (`נמאס לי ש...` /
     `פיצצו לי את המוח 🤯`, one expressive emoji near the top, no decorative
     bullets); flowing comma-chained sentences that read as thinking out loud
     (the *opposite* of one-line-paragraph broetry); self-aware hedges over hype;
     honestly names the hard part; concrete specifics (numbers, "weekend", "4th
     version"); tools named matter-of-factly; vivid/relatable close, never a CTA.
   - **Explainer / hot-take** (for external subjects): punchier, more structured
     (stat hook, bulleted mechanism, result numbers, "link in comments"), or a
     2–3 sentence sharp observation.
...
- LinkedIn broetry (every sentence its own one-line paragraph) — Gal's admired
  posts instead favor flowing comma-chained sentences; prefer that rhythm.
...
- Admired posts received (4 Hebrew, all by others) — stored as the target
  aesthetic / structural reference, not as Gal's voice. Topic seeds (Boris Cherny
  video, GTM migration, Slack→Monday agent, shared skill repo) still to be drafted.
- Seed few-shot examples: generated by drafting 1–2 real posts from Gal's listed
  topic ideas and capturing his edits (this is where his true voice calibrates).
